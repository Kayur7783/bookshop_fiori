...
module.exports = {
  ...
  cacheBSDestinations: function(req, cb){
    ...
    destinationUtils.getDestinationsByTenant(req.tenant, function(err,destinationsData){
      ...
      tenantDestinationBSCredentialsCache.set(req.tenant, destinationBSCredentials);
      return cb();
    });
  },
    ...
    return new Promise((resolve,reject) => {
      ...
      module.exports.getHTML5Applications(req, function(err,html5Applications){
        if (err){
          ...
          for (var i = 0; i < html5Applications.length; i++){
            if (html5Applications[i]['sap.cloud.service'].replace(/\./g, '') === serviceName &&
              html5Applications[i].applicationName === applicationName &&
              html5Applications[i].applicationVersion === applicationVersion){

              appHostId = html5Applications[i]['app-host-id'];
              applicationName = html5Applications[i].applicationName;
              break;
            }
          }
        }
